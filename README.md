...
- [アーキテクチャ(Step0〜7の実装範囲)](#アーキテクチャstep07の実装範囲)
- [セットアップ](#セットアップ)
- [品質ゲートの実行](#品質ゲートの実行)
- [pre-commitフック](#pre-commitフック)
- [ダミーログ生成(Step1)](#ダミーログ生成step1)
- [前処理・5分単位集計(Step2)](#前処理5分単位集計step2)
- [メタデータ層と生データ層の型分離(Step3)](#メタデータ層と生データ層の型分離step3)
- [異常検知と精度評価(Step4)](#異常検知と精度評価step4)
- [サイレント運用モード(Step5)](#サイレント運用モードstep5)
- [人間確認UI(Step6)](#人間確認uistep6)
- [LLM根本原因分析(Step7)](#llm根本原因分析step7)

## 実装状況

Step0(CIガードレール)〜Step7(LLM根本原因分析)まで全て実装済み。当初計画していた
ロードマップ(型分離→検知精度の実測→サイレント運用→人間確認→LLM根本原因分析という
段階的構成)は、これで一通り検証を終えている。

| Step | 内容 | 状態 |
|---|---|---|
| Step0 | CIガードレール(ruff/mypy/pytest/gitleaks/pip-audit) | 完了 |
| Step1 | ダミーログ生成 | 完了 |
| Step2 | 前処理・5分単位集計 | 完了 |
| Step3 | メタデータ層と生データ層の型分離 | 完了 |
| Step4 | 異常検知と精度評価(STL・IsolationForest) | 完了 |
| Step5 | サイレント運用モード | 完了 |
| Step6 | 人間確認UI(2段階開示) | 完了 |
| Step7 | LLM根本原因分析(Azure OpenAI) | 完了(実リソースへの接続確認は未実施、[LLM根本原因分析(Step7)](#llm根本原因分析step7)参照) |
