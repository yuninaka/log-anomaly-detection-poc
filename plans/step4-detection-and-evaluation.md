...
precisionが真に0.0(分母TP+FP>0で正しく算出、単に的中0件)なのに対し、recallの
「N/A」は分母(TP+FN)が0、つまりフィルタ後に実異常サンプルが1件も残らず評価不能
だったことを表す。`evaluation._safe_ratio`は分母0の場合0.0ではなくNaNを返し、
`evaluate_cli._format_ratio`が"N/A"として表示する(0.0だと「完全に検知失敗した」と
誤読されるため、「評価不能」と明確に区別する)。
...
### MIN_REQUEST_COUNT_FOR_EVALUATIONの位置づけ

この定数は`evaluate_cli.py`(評価専用スクリプト)にのみ存在し、`detection.py`の
検知関数には一切組み込まれていない。つまり「精度評価の母集団を統計的に妥当な
範囲に絞っただけ」であり、検知ロジック自体は低トラフィックバケットも含めて
全件にスコアを算出し続ける。実運用の検知パイプラインには影響しない。Step5の
サイレント運用モード設計時にこの位置づけを踏まえること。
